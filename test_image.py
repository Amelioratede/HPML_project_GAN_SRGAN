...
if __name__ == '__main__':
	...
	with torch.no_grad():
		...
		if opt.downsample == 'bicubic':
			size = img.size
			downscale = transforms.Resize((size[1]//upscale_factor,size[0]//upscale_factor), interpolation=Image.BICUBIC)
			img = downscale(img)
		...
		lr_upscale = transforms.Resize((h,w), interpolation=Image.NEAREST)
